...
CREATE DATABASE furniture_store_sales;
CREATE TABLE sales(ID INTEGER  NOT NULL  AUTO_INCREMENT PRIMARY KEY, transaction_date TEXT, product TEXT, price INTEGER, payment_type TEXT, name TEXT, city TEXT, state TEXT, country TEXT, account_created TEXT, last_login TEXT, latitude REAL, longitude REAL);
...
SELECT DATE(transaction_date) AS date, SUM(price) AS total_sales
FROM sales
GROUP BY date
ORDER BY date;

## WHAT IS THE AVG TRANSACTION VALUE PER PAYMENT TYPE
SELECT payment_type, AVG(price) AS average_transaction_value
FROM sales
...
## WHAT IS THE AVG NO. OF PURCHASES PER CUSTOMER
SELECT name, COUNT(*) AS number_of_purchases
FROM sales
GROUP BY name;
 
 
 ## WHAT ARE THE TOTAL SALES BY STATES ?
SELECT state, SUM(price) AS total_sales
FROM sales
GROUP BY state
ORDER BY total_sales DESC;


## HOW DOES THE SALES PERFORMANCE VARY BY COUNTRY ?
...
ORDER BY total_sales DESC;
 ...
 ## WHAT % OF TOTAL SALES IS MADE THROUGH EACH PAYMENT TYPE?
 SELECT payment_type, SUM(price) AS total_sales,(SUM(price) / (SELECT SUM(price) FROM sales)) * 100 AS percentage_of_total_sales
FROM sales
GROUP BY payment_type;


## WHICH PRODUCT HAVE THE HIGHEST AND LOWEST SALES?
SELECT product, SUM(price) AS total_sales
FROM sales
GROUP BY product
...
## HOW DO SALES VARY FOR A SPECIFIC PRODUCT ACROSS DIFFERENT REGION(CITIES /STATES)?
SELECT state, SUM(price) AS total_sales
FROM sales
WHERE product = 'Chair'
GROUP BY state
...
## OVERALL AVG, MAX, MIN VALUES FOR THE PRICE
SELECT 
...
## OVERALL AVG, MAX, MIN VALUES FOR THE PAYMENT TYPE
SELECT payment_type,
AVG(price) AS average_price,
MAX(price) AS max_price,
MIN(price) AS min_price
...
## WAYS TO GROUP DATA VALUES THAT DON'T EXIST YES (USING CASE) 
SELECT CASE
...
## OVERALL AVG, MAX, MIN VALUES FOR THE CITIES
 SELECT city,
AVG(price) AS average_price,
MAX(price) AS max_price,
MIN(price) AS min_price
